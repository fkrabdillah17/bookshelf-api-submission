...
const getAllBookHandler = (request, h)=> {
  ...
  if (reading){
    ...
    const filteredBookByReading = books.filter((book) => Number(book.reading) === isReading);
    const response = h.response({
      status: 'success',
      data: {
        books: filteredBookByReading.map((book)=> ({
          id: book.id, name: book.name, publisher: book.publisher,
        })),
      },
    });
  ...
  const filteredBookByFinished = books.filter((book) => Number(book.finished) === isFinished);
  const response = h.response({
    status: 'success',
    data: {
      books: filteredBookByFinished.map((book)=> ({
        id: book.id, name: book.name, publisher: book.publisher,
      })),
    },
  });
